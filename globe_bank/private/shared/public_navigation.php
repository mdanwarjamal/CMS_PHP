  ...
  <ul class="subjects">
    <?php while($nav_subject = mysqli_fetch_assoc($nav_subjects)) { ?>
      ...
      <li class="<?php if($nav_subject['id'] == $subject_id){echo 'selected';}?>">
        <?php $first_page_id = find_first_page_id_with_subject_id($nav_subject['id'],['visible'=>$visible]); ?>
        <a href="<?php echo url_for('index.php?subject_id=' . h(u($nav_subject['id']))) . "&id=" . h(u($first_page_id)); ?>">
          <?php echo h($nav_subject['menu_name']); ?>
        </a>
        <?php

          $nav_pages = find_pages_by_subject_id($subject_id,['visible'=>$visible]);
        ?>
